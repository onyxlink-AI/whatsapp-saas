...
// One-off: creates the "recordatorio_cita" template as a draft for the
// Onyxlink workspace and submits it to YCloud/Meta for approval. Needed
// because the appointment-reminders cron has nothing to send without at
// least one approved template (utility category — transactional reminder).
...
async function main() {
  ...
  const templateInput: CreateTemplateInput = {
    ...
    header_text: "",
    body_template:
      "Hola {{1}}, este es un recordatorio: tienes una cita programada hoy a las {{2}}. ¡Te esperamos!",
    body_variables: [
