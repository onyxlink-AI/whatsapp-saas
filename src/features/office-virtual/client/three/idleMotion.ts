...
export function resolveIdleMotion(elapsedSeconds: number, agentPhase: number, routeDistance: number): IdleMotion {
  ...
  for (let trip = 0; trip < 256; trip += 1) {
    ...
    if (remaining < deskPause) return { phase: 'at-desk', progress: 0 };
    remaining -= deskPause;
    if (remaining < outwardWalk) return { phase: 'walking-out', progress: remaining / outwardWalk };
    remaining -= outwardWalk;
    if (remaining < cafePause) return { phase: 'at-cafe', progress: 1 };
    remaining -= cafePause;
    if (remaining < returnWalk) return { phase: 'walking-back', progress: 1 - remaining / returnWalk };
    remaining -= returnWalk;
  }

  return { phase: 'at-desk', progress: 0 };
}
