...
describe("resolveEntitlements", () => {
  ...
  it("whatsapp_oficina: whatsapp + oficina + Board, SIN gestión (Paquete 4)", () => {
  ...
  it("whatsapp: solo whatsapp + Board, SIN gestión ni oficina (Paquete 5)", () => {
  ...
  it("oficina: solo oficina + Board, SIN gestión ni whatsapp (Paquete 6)", () => {
...
describe("lostCapabilities", () => {
  ...
  it("gestion -> whatsapp_oficina loses gestión aunque gane whatsapp+oficina (cambio lateral, no un upgrade limpio)", () => {
    expect(lostCapabilities("gestion", "whatsapp_oficina")).toEqual([
      "Clientes, Proyectos, Agenda, Anotaciones y Contenido",
    ]);
  });

  it("whatsapp_gestion -> whatsapp_oficina loses solo gestión, conserva whatsapp/Board y gana oficina", () => {
    expect(lostCapabilities("whatsapp_gestion", "whatsapp_oficina")).toEqual([
      "Clientes, Proyectos, Agenda, Anotaciones y Contenido",
    ]);
  });

  it("suite -> whatsapp_oficina loses solo gestión", () => {
    expect(lostCapabilities("suite", "whatsapp_oficina")).toEqual([
      "Clientes, Proyectos, Agenda, Anotaciones y Contenido",
    ]);
  });

  it("whatsapp_oficina -> whatsapp loses solo oficina", () => {
    expect(lostCapabilities("whatsapp_oficina", "whatsapp")).toEqual(["Oficina Virtual"]);
  });

  it("whatsapp_oficina -> oficina loses solo whatsapp", () => {
    expect(lostCapabilities("whatsapp_oficina", "oficina")).toEqual(["Agente de WhatsApp y Conversaciones"]);
  });

  it("suite -> whatsapp (Paquete 5) loses gestión y oficina, conserva Board", () => {
  ...
  it("suite -> oficina (Paquete 6) loses gestión y whatsapp, conserva Board", () => {
...
describe("PACKAGE_MATRIX", () => {
  ...
  it("cada paquete no-'none' concede exactamente uno de los 7 cubos conocidos — nunca una combinación intermedia distinta", () => {
    ...
    expect(PACKAGE_MATRIX.whatsapp).toEqual({ hasGestion: false, hasWhatsappAgent: true, hasOfficeVirtual: false, hasWhiteboard: true });
    expect(PACKAGE_MATRIX.oficina).toEqual({ hasGestion: false, hasWhatsappAgent: false, hasOfficeVirtual: true, hasWhiteboard: true });
    expect(PACKAGE_MATRIX.whatsapp_oficina).toEqual({ hasGestion: false, hasWhatsappAgent: true, hasOfficeVirtual: true, hasWhiteboard: true });
