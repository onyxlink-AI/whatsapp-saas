...
// ──────────────────────────────────────────────────────────────────────────────
// acceptPipelineSuggestion — applies a pending AI stage suggestion (Sugerencias
// de Pipeline con IA) and clears it. Reuses moveDealStage's exact update shape.
// ──────────────────────────────────────────────────────────────────────────────
export async function acceptPipelineSuggestion(
  dealId: string,
  ...
  const { data: deal, error: dealError } = await supabase
    .from("deals")
    .select("workspace_id, ai_suggested_stage")
    .eq("id", dealId)
    .single();

  if (dealError || !deal || !deal.ai_suggested_stage) {
    return { ok: false, error: "No hay sugerencia pendiente para este deal" };
  }

  const targetStage = deal.ai_suggested_stage as DealStage;
  ...
  const { data: updated, error: updateError } = await supabase
    ...
    .update({
      stage: targetStage,
      position: count ?? 0,
      closed_at: CLOSED_STAGES.includes(targetStage)
        ? new Date().toISOString()
        : null,
  ...
  if (updateError || !updated) {
    console.error(
      "[acceptPipelineSuggestion] Supabase error:",
      updateError?.message,
    );
    return { ok: false, error: "Error al aplicar la sugerencia" };
  }

  return { ok: true, data: { id: updated.id as string, stage: targetStage } };
}

// ──────────────────────────────────────────────────────────────────────────────
// dismissPipelineSuggestion — clears a pending AI stage suggestion without
// applying it.
// ──────────────────────────────────────────────────────────────────────────────
export async function dismissPipelineSuggestion(
  dealId: string,
): Promise<ActionResult<{ id: string }>> {
  const supabase = await createClient();
  const {
  ...
  if (updateError || !updated) {
    console.error(
      "[dismissPipelineSuggestion] Supabase error:",
      updateError?.message,
    );
    return { ok: false, error: "Error al descartar la sugerencia" };
  }
