...
/**
 * task-actions.ts — Server actions for follow-up task CRUD, completion,
 * and reassignment. A task must be linked to a deal and/or a contact.
 */

import { z } from "zod";
import { createClient } from "@/lib/supabase/server";
import type { TaskRow, TaskStatus } from "@/features/pipeline/types";
import {
  CreateTaskSchema,
  UpdateTaskSchema,
...
export async function createTask(
  ...
  // Resolve workspace_id from whichever parent entity was given.
  const workspaceId = await resolveWorkspaceId(supabase, parsed.data);
  if (!workspaceId) {
    return { ok: false, error: "No se pudo resolver el workspace" };
  }
...
async function resolveWorkspaceId(
  ...
  data: { deal_id?: string; contact_id?: string },
): Promise<string | null> {
  if (data.deal_id) {
    const { data: deal } = await supabase
      ...
      .eq("id", data.deal_id)
      .single();
    if (deal) return deal.workspace_id as string;
  }
  ...
  if (data.contact_id) {
    const { data: contact } = await supabase
      ...
      .single();
    if (contact) return contact.workspace_id as string;
  }
